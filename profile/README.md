...
| 이름 | 역할 | 주요 담당 업무 |
|---|---|---|
| 김예원 | 전체 팀장 · 데이터/백엔드 팀장 | 데이터 수집·전처리·임베딩·BERTopic 분석 총괄, FastAPI·Firebase·DB·API·WebSocket 설계 및 프론트엔드 연동 |
| 김주영 | 기획 팀장 | 문제·고객·서비스 정의, 요구사항 정의서, IA·플로우·유스케이스, 비즈니스 캔버스·성과지표·WBS 기획 |
| 김채원 | 디자인 팀장 · 프론트엔드 | 브랜드·UX/UI 총괄, 온보딩·홈·산책·리포트·마이페이지 화면 설계 및 Flutter 프론트엔드 구현, TalkBack 접근성 적용, 제품·서비스 영상 시각화 |
| 신채연 | 최종 산출 보고서 팀장 | 리서치·문서 취합, 요구사항·다이어그램·화면 설계 지원, BX·CX·DX·CAM·시스템 아키텍처 서술 및 최종 보고서 완성 |
| 황병관 | 카메라·모델링 팀장 | YOLO 데이터 구축·v1–v4 학습과 모델 변환, 카메라 송·수신 앱 개발, AI–프론트엔드 통합 및 하드웨어 프로토타입 제작 |
